...
void Sounds::playMunch()
{
	if (enabled)
	{
		...
		{
			if (chunk_munch_a)
				channel_munch = Mix_PlayChannel(channel_munch, chunk_munch_a, 0.75);
		}
		else
		{
			if (chunk_munch_b)
				channel_munch = Mix_PlayChannel(channel_munch, chunk_munch_b, 0.75);
		}
		if (channel_munch == -1)
		{
...
Mix_Chunk *Sounds::loadWaveSound(const char *filePath)
{
	Mix_Chunk *sound = Mix_LoadWAV(filePath);
	if (!sound)
	{
		std::cerr << "Unable to load wave sound file (" << filePath << "): " << Mix_GetError() << std::endl;
	}
	return sound;
}

Mix_Music *Sounds::loadWaveMusic(const char *filePath)
{
	Mix_Music *music = Mix_LoadMUS(filePath);
	if (!music)
	{
		std::cerr << "Unable to load wave music file (" << filePath << "): " << Mix_GetError() << std::endl;
	}
	return music;
}
